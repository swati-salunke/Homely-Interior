...
const Contact = forwardRef(({ }, ref) => {
  const [isScrolledToContact, setIsScrolledToContact] = useState(!ref || !ref.current);

  useEffect(() => {
    if (ref && ref.current && !isScrolledToContact) {
      ref.current.scrollIntoView({ behavior: 'smooth' });
      setIsScrolledToContact(true);
    }
  }, [ref, isScrolledToContact]);

  return (
    <>
      <div ref={ref} className="sm:flex flex-row w-full bg-gray-100 justify-center sm:px-24 px-4 py-28 gap-8 h-auto">
